...
def detect_circles(target_img):
   ...
   # mid_x = np.median(x[~ np.isnan(x)])
   # mid_y = np.median(y[~ np.isnan(y)])
    ...
    print (length)
    step = int((length / 11) + 0.49)
    print (step)
    i=1
    while i <= length:
    #for i in range(10):
       x1.append(indices[0][i])
       y1.append(indices[1][i])
       i = i+2
...
def detect_dark_rectangle(img, circles):  
   ...
   # if x_circle + r > width or x_circle - r < 0 or y_circle + r > height or y_circle - c < 0:
   #    print ('test2')
   #    return None
   # if circles is None or img is None:
   #    print ('test1')
    while x_circle + r >= width or x_circle - r <=0:
       r = r - 8
    while y_circle + c >= height or y_circle - c <=0:
       c = c - 8
    print ('c,r')
    print (c,r)

    roi = img[y_circle - c:y_circle + c, x_circle -r:x_circle + r]
    print (roi.shape)
